---
title: Mermaid 图表
description: 学习如何在 Xinyi Class Markdown 页面中创建 Mermaid 图表。
navigation:
  icon: i-lucide-chart-network
  position: 5
seo:
  title: Mermaid 图表
  description: 使用围栏代码块在 Xinyi Class 内容中渲染 Mermaid 图表。
---

## Mermaid 已默认启用

Xinyi Class 在共享层中注册了 `@barzhsieh/nuxt-content-mermaid`，因此 `docs/content/**` 下的任何 Markdown 页面都可以渲染 Mermaid 围栏代码块。
...
flowchart TD
  ...
  B --> C[Xinyi Class 将其转换为 `<Mermaid>`]
  C --> D[Nuxt 渲染 SVG]
```

## 构建适合 Xinyi Class 的图表

一个简单的 Xinyi Class 示例：
...
Xinyi Class 为 Mermaid 覆盖提供了 `config` frontmatter 字段。共享层已经在 `layer/content.config.ts` 中声明了此字段，因此它会被解析为对象。
