---
title: LLMs 集成
description: Xinyi Class 使用 Nuxt LLMs 模块生成 AI 就绪的内容文件
navigation:
  icon: i-lucide-message-circle-code
---

Xinyi Class 默认集成 `nuxt-llms`，为大型语言模型（LLMs）准备您的内容。所有文档页面都会被注入，并自动生成和预渲染 `/llms.txt` 和 `/llms-full.txt` 文件。

::note
查看 Xinyi Class 站点部署后生成的 `/llms.txt` 文件。
::
...
当启用 `nuxt-llms` 时，Xinyi Class 会通过规范的 `.md` 页面 URL 暴露原始预处理 Markdown/MDC，这样 AI 工具就不必读取冗长的后处理输出。
...
::note
直接 `.md` 页面 URL 在所有环境中都可用。在具备服务器运行时的部署中，当 Agent 发送偏好 Markdown 的请求头时，Xinyi Class 也会直接协商返回 Markdown 响应。在 Vercel 上，构建时注入的路由规则会让预渲染页面保持相同行为。
::

在具备服务器运行时的部署中，当请求偏好 Markdown 响应时，Xinyi Class 会自动将 Markdown 内容提供给 AI Agent 和 CLI 工具。
...
Xinyi Class 使用 HTTP 头部检测来自 AI Agent 和命令行工具的请求：
