...
Xinyi Class 自动发现 `skills/` 目录中的技能，并在 `/.well-known/skills/` 下提供服务，遵循 [Cloudflare Agent Skills Discovery RFC](https://github.com/cloudflare/agent-skills-discovery-rfc)。这使得您的技能可以通过一条命令从任何文档 URL 安装。
...
::note
当 Xinyi Class 站点提供技能时，其目录会暴露在 `/.well-known/skills/index.json`。
::
...
在 Xinyi Class 项目根目录添加一个 `skills/` 目录，并在其中创建包含 `SKILL.md` 文件的技能子目录：
...
部署您的文档。Xinyi Class 会自动在 `/.well-known/skills/` 下提供您的技能。
...
默认情况下，Xinyi Class 在项目根目录的 `skills/` 目录中查找技能。您可以在 `nuxt.config.ts` 中使用 `tockdocs.skills.dir` 更改此设置：
...
Xinyi Class 在构建时扫描您的 `skills/` 目录并生成两种类型的端点：
