...
每个 Xinyi Class 实例都包含一个内置的 MCP 服务器，因此 Claude、Cursor、VS Code、Windsurf 和 Zed 等 AI 工具可以搜索和阅读您的文档。
...
Xinyi Class MCP 服务器使用 HTTP 传输，可以连接到多个助手。
...
Xinyi Class 使用 `@nuxtjs/mcp-toolkit`，因此您可以使用自定义工具、资源、提示词和处理程序扩展 MCP 服务器。
