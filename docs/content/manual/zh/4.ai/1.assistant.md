...
Xinyi Class 助手是一个**基于文档的对话 UI**，构建于以下技术之上：
...
2. 服务器解析活动的提供商和模型。
3. 当请求来自文档页面时，Xinyi Class 会尽可能将请求范围限定到当前的**知识库**和**语言**。
4. 助手向模型暴露 MCP 工具。
...
Xinyi Class 支持 **Vercel AI Gateway**，以及：
...
如果 `AI_PROVIDER` 未设置，Xinyi Class 会从您的可用凭据中自动检测第一个已配置的提供商。
...
export default defineAppConfig({
  assistant: {
    faqQuestions: [
      {
        category: '快速入门',
        items: ['如何安装 Xinyi Class？', '项目结构是怎样的？'],
      },
      {
        category: '自定义',
...
助手端点仅接受 `POST` 请求。它会拒绝跨源浏览器请求，在调用模型前验证请求体，并按客户端 IP 对请求进行速率限制。

在知识库模式下，服务器会优先从同源页面 referer 解析作用域，然后再使用客户端发送的 `X-TockDocs-KB` 和 `X-TockDocs-Locale` 请求头。没有有效知识库作用域的请求会在检索前被拒绝。

可以使用仅限服务器的环境变量调整默认保护规则：

| 变量 | 默认值 | 描述 |
| --- | ---: | --- |
| `ASSISTANT_RATE_LIMIT_MAX_REQUESTS` | `20` | 每个客户端 IP 在一个窗口内允许的最大助手请求数。 |
| `ASSISTANT_RATE_LIMIT_WINDOW_MS` | `60000` | 速率限制窗口长度，单位为毫秒。 |
| `ASSISTANT_MAX_BODY_BYTES` | `131072` | 最大 JSON 请求体大小。 |
| `ASSISTANT_MAX_MESSAGES` | `20` | 每个请求允许的最大聊天消息数。 |
| `ASSISTANT_MAX_MESSAGE_TEXT_CHARS` | `8000` | 单条消息允许的最大文本长度。 |
| `ASSISTANT_MAX_TOTAL_TEXT_CHARS` | `24000` | 所有消息合计允许的最大文本长度。 |
