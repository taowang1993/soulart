...
seo:
  description: 学习如何使用 Xinyi Class 和 Nuxt Content 编写文档。
  title: 内容编辑
---

Xinyi Class 基于 **Nuxt Content** 构建。您使用 Markdown 编写文档，通过 **MDC 语法** 添加更丰富的 UI，由层根据您的内容结构自动生成路由、导航、搜索和页面元数据。

## 内容模式

Xinyi Class 支持两种路由/内容架构。
...
当 Xinyi Class 检测到一个或多个 `content/<kb>/kb.yml` 文件时，知识库模式即被启用。
...
如果没有自定义 Vue 首页，Xinyi Class 使用 Markdown 首页内容：
...
- 否则 Xinyi Class 回退到知识库目录页面

### 自定义 Vue 首页

由于 Xinyi Class 是一个 Nuxt 层，您始终可以用自己的 Vue 页面替换首页：
...
Xinyi Class 在生成公共 URL 时会忽略数字排序前缀。例如：
