...
description: Learn how to create Mermaid diagrams in Xinyi Class Markdown pages.
navigation:
  icon: i-lucide-chart-network
  position: 5
seo:
  title: Mermaid diagrams
  description: Render Mermaid diagrams in Xinyi Class content with fenced code blocks.
---

## Mermaid is already enabled

Xinyi Class registers `@barzhsieh/nuxt-content-mermaid` in the shared layer, so any Markdown page under `docs/content/**` can render Mermaid fences.
...
flowchart TD
  ...
  B --> C[Xinyi Class converts it to `<Mermaid>`]
  C --> D[Nuxt renders the SVG]
```

## Build diagrams that fit Xinyi Class

A simple Xinyi Class example:
...
Xinyi Class exposes a `config` frontmatter field for Mermaid overrides. The shared layer already declares this field in `layer/content.config.ts`, so it is parsed as an object.
