...
The Xinyi Class assistant is a **docs-grounded chat UI** built on top of:
...
3. When the request comes from a docs page, Xinyi Class scopes the request to the current **knowledge base** and **locale** when possible.
...
Xinyi Class supports **Vercel AI Gateway**, plus:
...
If `AI_PROVIDER` is unset, Xinyi Class auto-detects the first configured provider from your available credentials.
    ...
    faqQuestions: [
      {
        ...
        items: ['How do I install Xinyi Class?', 'What is the project structure?'],
      },
      {
...
The assistant endpoint accepts `POST` requests only. It rejects cross-origin browser requests, validates the request body before invoking a model, and rate-limits requests by client IP.

In knowledge-base mode, the server resolves scope from the same-origin page referer first, then from the client-sent `X-TockDocs-KB` and `X-TockDocs-Locale` headers. Requests without a valid knowledge-base scope are rejected before retrieval.

You can tune the default guardrails with server-only environment variables:

| Variable | Default | Description |
| --- | ---: | --- |
| `ASSISTANT_RATE_LIMIT_MAX_REQUESTS` | `20` | Maximum assistant requests per client IP per window. |
| `ASSISTANT_RATE_LIMIT_WINDOW_MS` | `60000` | Rate-limit window length in milliseconds. |
| `ASSISTANT_MAX_BODY_BYTES` | `131072` | Maximum JSON body size. |
| `ASSISTANT_MAX_MESSAGES` | `20` | Maximum number of chat messages per request. |
| `ASSISTANT_MAX_MESSAGE_TEXT_CHARS` | `8000` | Maximum text length for any single message. |
| `ASSISTANT_MAX_TOTAL_TEXT_CHARS` | `24000` | Maximum combined message text length. |
