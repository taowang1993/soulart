...
description: Xinyi Class generate AI-ready content files using Nuxt LLMs module
navigation:
  icon: i-lucide-message-circle-code
---

Xinyi Class integrates `nuxt-llms` by default to prepare your content for Large Language Models (LLMs). All your documentation pages are injected and `/llms.txt` and `/llms-full.txt` files are automatically generated and pre-rendered.

::note
Have a look at the generated `/llms.txt` file on a deployed Xinyi Class site.
::
...
When `nuxt-llms` is enabled, Xinyi Class exposes the original source markdown/MDC through canonical `.md` page URLs, so AI tools can avoid the verbose post-processed output.
...
::note
Direct `.md` page URLs work in every environment. On server-backed deployments, Xinyi Class also negotiates markdown responses from standard page URLs when agents send markdown-friendly headers. On Vercel, build-time route rules keep prerendered pages behaving the same way.
::

On server-backed deployments, Xinyi Class automatically serves markdown content to AI agents and CLI tools when requests prefer markdown responses.
...
Xinyi Class detects requests from AI agents and command-line tools using HTTP headers:
