...
Every Xinyi Class instance includes a built-in MCP server so AI tools such as Claude, Cursor, VS Code, Windsurf, and Zed can search and read your documentation.
...
Xinyi Class exposes three built-in tools.
...
The Xinyi Class MCP server uses HTTP transport and can be connected to multiple assistants.
...
Xinyi Class uses `@nuxtjs/mcp-toolkit`, so you can extend the MCP server with custom tools, resources, prompts, and handlers.
