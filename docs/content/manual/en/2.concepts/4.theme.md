---
title: Theme
description: Custom the appearance of your Xinyi Class documentation thanks to Nuxt UI flexible theming.
navigation:
  icon: i-lucide-paint-roller
---

Xinyi Class is built on top of Nuxt UI and takes full advantage of Tailwind CSS v4, CSS variables. The Tailwind Variants API offers a flexible and scalable theming system.
...
Xinyi Class uses pre-configured color aliases that are used to style components and power the `color` props across the UI.
...
## Xinyi Class Subcomponents

Xinyi Class uses several Nuxt UI components internally for navigation, table of contents, and sub-navigation. You can customize their default variants through `app.config.ts` using the `defaultVariants` key, without having to override the entire component.
