---
title: Configuration
description: "Customize your Xinyi Class documentation from the Nuxt application configuration file. "
navigation:
  icon: i-lucide-settings
seo:
  description: Customize your Xinyi Class documentation from Nuxt application configuration file.
  title: Configuration
---

Xinyi Class allows you to configure your documentation through the [app.config.ts](https://nuxt.com/docs/guide/directory-structure/app-config) file provided by Nuxt.
...
Technical SEO is tricky and boring. Xinyi Class offers a solid, opt-in default setup that works out of the box, while giving you full control to customize your SEO metadata, from page titles to social sharing images.

### Metadata

Xinyi Class offers flexible `SEO` metadata configuration, allowing you to easily override values globally or on a per-page basis.
...
:site-image{.rounded-md.w-full alt="og image documentation page" src="/_og/s/o_s0putq.png"}
...
Since Xinyi Class is a [Nuxt layer](https://nuxt.com/docs/guide/going-further/layers), you can override docs or landing page OG image by creating a file with the same name in your own `components/OgImage/` directory.
...
Xinyi Class automatically generates a sitemap at `/sitemap.xml` containing all your documentation pages. This helps search engines discover and index your content.
...
By default, Xinyi Class includes a color mode toggle in the header and footer, allowing users to switch between light and dark modes.
...
- Built-in Xinyi Class interface strings
...
Xinyi Class reads your `.git/` folder to get the `url` and `branch` of your repository to add:
...
export default defineAppConfig({
  github: {
    url: 'https://github.com/taowang1993/soulart',
    branch: 'main',
    rootDir: 'docs',
  },
})
