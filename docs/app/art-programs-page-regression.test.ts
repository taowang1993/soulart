...
test('art programs page keeps reference card image treatments', () => {
  ...
  assert.match(source, /#children article:first-child \.child-photo-frame img[\s\S]*object-fit: contain/)
  assert.match(source, /\.teen-grid article:first-child \.teen-photo-frame[\s\S]*aspect-ratio: 1/)
  assert.match(source, /\.teen-grid article:first-child \.teen-photo-frame img[\s\S]*transform: none/)
